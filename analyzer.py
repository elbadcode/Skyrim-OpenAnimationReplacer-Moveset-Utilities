        ...
        for root, dirs, files in os.walk(dar_path, topdown=False):
            for _dir in dirs:
                ...
                for file in _dir:
                    try:
                        if file.endswith(".hkx"):
                            files_found = True
                            for _file in _dir:
                                if _file.endswith("_conditions.txt"):
                                    config_found = True
                    except UnboundLocalError:
                        print('unbound')
                dir_priority = 0
                try:
                    dir_priority = int(_dir)
                except TypeError:
                    print(_dir)
                    ...
                    for grip in grips:
                        if grip in _dir:
                            is_grip = grip
                    for keytrace in keytraces:
                        if keytrace in _dir:
                            is_keytrace = keytrace
                ...
                if config_found:
                    ...
                    if os.path.exists(config_file_path):
                        config_file_path = config_file_path.lower()
                        conditions_file_path = config_file_path.lower()
