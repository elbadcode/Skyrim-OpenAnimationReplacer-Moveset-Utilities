            ...
            for filename in filenames:
                if filename == "config.json":
                    ...
                    with open(filepath, "r") as file:
                        ...
                        match = re.search(r"\"Keytrace\.esp\"\,\n\s+\"formID\":\s\"([0-9A-F]{3})", content)
                        match2 = re.search(
                            r"\"Stances\s\-\sDynamic\sWeapon\sMovesets\sSE\.esp\"\,\n\s+\"formID\":\s\"4251([0-9A-F]{1})",
                            content)
                        if match:
                                ...
                                if old_dir != new_name:
                                    ...
                                    for _file in os.listdir(old_dir):
                                        if _file.endswith(".hkx"):
                                            ...
                                            new_file = os.path.join(new_dir, base_filename)
                                            print(new_file)
                                            shutil.move(os.path.join(old_dir, _file), os.path.join(new_dir, new_file))
                            organize_files(new_dir)
                            os.chdir(os.path.dirname(parent_path))
                        elif match2:
                            #
                            ...
                            key_code = ''
                            if match2.group(2).lower() == 'a':
                                key_code = 'A'
                            # A regular else statement will still continue in python so its either this or catch the exception from trying to convert a to int
                            elif match2.group(2).lower() != 'a':
                                key_code = int(match2.group(2))

                            print('key_code ')
                            if key_code in KEY_MAPPINGS_STANCE:
                                new_name = KEY_MAPPINGS_STANCE[key_code]
                                old_dir = dirpath
                                new_dir = os.path.join(os.path.dirname(dirpath), new_name)
                                if old_dir != new_name:
                                    print(f"Renaming folder '{old_dir}' to '{new_name}'")
